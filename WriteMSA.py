...
def writeMSA(countFile, refSeq, raCutoff, outputFile):
    ...
    for line in lines:
        ...
        if spt[0]=='0':
            ...
            wtActivity = partCount/fullCount
    ...
    for line in lines:
        ...
        fullCount = float(spt[2])
        partCount = float(spt[3])
        if fullCount < minCount:
            continue
        RA = partCount/fullCount/wtActivity
        if RA < raCutoff:
            continue
...
def argParseInit():
    ...
    parser.add_argument('--minCount', default='50', help='Relative Activity cutoff')
